...
import { collection, doc, setDoc, deleteDoc, onSnapshot, getDocFromServer, writeBatch } from 'firebase/firestore';
...
export default function App() {
  ...
  // Refs to prevent onSnapshot from overwriting optimistic state during writes
  const isWritingOrders = useRef(false);
  ...
  const [selectedGender, setSelectedGender] = useState<'all' | 'men' | 'women' | 'sale'>('all');
  ...
  useEffect(() => {
    // Hash routing fallback to clean HTML5 URL routing path mapping
    if (window.location.hash) {
      const hashPath = window.location.hash.substring(1); // remove '#'
      if (hashPath === '/products' || hashPath === '/collection' || hashPath === 'products' || hashPath === 'collection') {
        window.history.replaceState(null, '', '/');
        // Smooth scroll to collection grid
        setTimeout(() => {
          const collEl = document.getElementById('collection');
          if (collEl) collEl.scrollIntoView({ behavior: 'smooth' });
        }, 100);
      } else if (hashPath === '/cart' || hashPath === 'cart') {
        setIsCartOpen(true);
        window.history.replaceState(null, '', '/cart');
      } else if (hashPath === '/checkout' || hashPath === 'checkout') {
        setIsCheckoutOpen(true);
        window.history.replaceState(null, '', '/checkout');
      } else if (hashPath === '/admin' || hashPath === 'admin') {
        setIsAdminOpen(true);
        window.history.replaceState(null, '', '/admin');
      } else if (hashPath === '/track' || hashPath === 'track') {
        setIsOrderTrackerOpen(true);
        window.history.replaceState(null, '', '/track');
      } else if (hashPath.startsWith('/products/')) {
        const pId = hashPath.split('/products/')[1];
        window.history.replaceState(null, '', `/products/${pId}`);
      } else if (hashPath.startsWith('products/')) {
        const pId = hashPath.split('products/')[1];
        window.history.replaceState(null, '', `/products/${pId}`);
      }
  ...
  /**
   * Sanitize a product before writing to Firestore.
   * - Strips base64 data: URIs (too large for Firestore's 1MB doc limit → causes "invalid nested entity" error)
   * - Removes undefined values from nested arrays (Firestore rejects undefined)
   */
  const sanitizeProductForFirestore = (p: Product): Record<string, any> => {
    const stripBase64 = (url: string | undefined): string => {
      if (!url) return '';
      if (url.startsWith('data:image/')) return ''; // Strip base64
      return url;
    };
    ...
    const sanitizedColors = (p.colors || []).map(c => ({
      name: c.name ?? '',
      value: c.value ?? '',
      imageUrl: stripBase64(c.imageUrl),
    }));

    const sanitizedReviews = (p.reviews || []).map(r => ({
      id: r.id ?? '',
      rating: r.rating ?? 5,
      comment: r.comment ?? '',
      author: r.author ?? '',
    ...
    const sanitizedImages = (p.images ?? []).map(stripBase64).filter(Boolean);
    const hoverImageUrl = stripBase64(p.hoverImageUrl);
  ...
  useEffect(() => {
    ...
    const unsubscribeWhatsApp = onSnapshot(doc(db, 'whatsapp_config', 'main'), (snap) => {
      if (snap.exists() && snap.data().number) {
        const currentNum = snap.data().number;
        setWhatsappNumber(currentNum);
        // Auto-update Firestore if it has the old dummy number
        if (auth.currentUser && (currentNum === '+94712345678' || currentNum === '94712345678')) {
    ...
    const unsubscribeProducts = onSnapshot(collection(db, 'products'), (snap) => {
      ...
      if (snap.empty) {
        // Only seed INITIAL_PRODUCTS on the very first run of the app.
        // Use a localStorage flag 'reed_seeded' to track whether initial seeding has already happened.
        // If the flag is set, it means an admin intentionally deleted all products — do NOT re-seed.
        const hasBeenSeeded = localStorage.getItem('reed_seeded');

        if (!hasBeenSeeded) {
          // First ever run: seed initial products
          setProducts(INITIAL_PRODUCTS);
          localStorage.setItem('reed_products', JSON.stringify(INITIAL_PRODUCTS));
          localStorage.setItem('reed_seeded', 'true'); // Mark as seeded so we never auto-seed again
          if (auth.currentUser) {
            INITIAL_PRODUCTS.forEach((p) => {
              setDoc(doc(db, 'products', p.id), sanitizeProductForFirestore(p)).catch(console.error);
            });
          }
        } else {
          // Admin intentionally cleared all products — respect that decision, show empty catalog
          setProducts([]);
          localStorage.setItem('reed_products', JSON.stringify([]));
        }
      } else {
        ...
        localStorage.setItem('reed_products', JSON.stringify(firestoreProducts));
        // Ensure the seeded flag is always set once we have real products
        localStorage.setItem('reed_seeded', 'true');
      }
      setIsLoading(false);
    }, (error) => {
      console.error('Firestore products listener error:', error);
      // On Firestore error, fall back to localStorage
      const localProducts = localStorage.getItem('reed_products');
      if (localProducts) {
    ...
    if (savedCart) {
      try {
        ...
        if (Array.isArray(parsed)) {
          // Validate cart item schema to prevent corruption crashes
    ...
    if (firebaseUser != null) {
      ...
      if (savedOrders) {
        try {
          ...
          if (Array.isArray(parsed)) {
            // Validate orders schema to prevent corruption crashes
            const valid = parsed.filter(o => o && typeof o.orderId === 'string');
            setOrders(valid);
          }
        } catch (e) {
  ...
  /**
   * Scans all images in the product list.
   * If any image is still a base64 data URI, uploads it to Firebase Storage
   * and replaces it with the returned permanent cloud URL.
   * This ensures images are never silently dropped by sanitizeProductForFirestore.
   */
  const uploadProductImagesToStorage = async (products: Product[]): Promise<Product[]> => {
    const uploadIfBase64 = async (url: string | undefined, hint: string): Promise<string> => {
      if (!url) return '';
      if (!url.startsWith('data:image/')) return url; // Already a URL — nothing to do
      try {
        console.log(`[uploadProductImagesToStorage] Uploading base64 image to Storage: ${hint}`);
        return await uploadBase64ToStorage(url, hint);
      } catch (err) {
        console.error(`[uploadProductImagesToStorage] Failed to upload ${hint}:`, err);
        return ''; // Drop the image on failure rather than crash
      }
    };

    // Process all products in parallel for speed
    return Promise.all(
      products.map(async (p) => {
        // Upload main images array
        const uploadedImages = await Promise.all(
          (p.images ?? []).map((img, i) => uploadIfBase64(img, `${p.id}_img${i}`))
        );

        // Upload hover image
        const uploadedHover = await uploadIfBase64(p.hoverImageUrl, `${p.id}_hover`);

        // Upload color variant images
        const uploadedColors = await Promise.all(
          (p.colors ?? []).map(async (c) => ({
            ...c,
            imageUrl: await uploadIfBase64(c.imageUrl, `${p.id}_color_${c.name}`),
          }))
        );
        ...
        return {
          ...
          hoverImageUrl: uploadedHover || undefined,
  ...
  const updatePersistentProducts = async (updatedPrs: Product[]) => {
    // Capture the current product IDs BEFORE updating state (avoids stale closure)
    const previousIds = products.map(p => p.id);

    console.log('[updatePersistentProducts] start. previousIds:', previousIds);
    console.log('[updatePersistentProducts] updatedPrs IDs:', updatedPrs.map(p => p.id));

    // Update state immediately for instant UI feedback (with original data incl. base64)
    setProducts(updatedPrs);

    // ALWAYS persist to localStorage so products survive refresh regardless of auth/Firestore status
    // localStorage keeps full data including base64 images for offline/local display
    localStorage.setItem('reed_products', JSON.stringify(updatedPrs));

    // Attempt Firestore write — will succeed if user is Firebase-authenticated
    try {
      // --- CLOUD IMAGE UPLOAD STEP ---
      // Before saving to Firestore, upload any base64 images to Firebase Storage.
      // sanitizeProductForFirestore() strips base64 to avoid Firestore's 1MB limit,
      // so we must upload them first and replace with cloud URLs.
      const prsWithCloudImages = await uploadProductImagesToStorage(updatedPrs);

      // Update state & localStorage again with the cloud image URLs
      setProducts(prsWithCloudImages);
      localStorage.setItem('reed_products', JSON.stringify(prsWithCloudImages));

      const activeIds = new Set(prsWithCloudImages.map(p => p.id));
      const batch = writeBatch(db);
      let changesCount = 0;

      // 1. Delete products that are no longer in the updated list
      for (const oldId of previousIds) {
        if (!activeIds.has(oldId)) {
          console.log('[updatePersistentProducts] Deleting product from Firestore:', oldId);
          batch.delete(doc(db, 'products', oldId));
          changesCount++;
        }
      }

      // 2. Identify additions and edits by comparing with current state
      for (const p of prsWithCloudImages) {
        const existingProd = products.find(oldP => oldP.id === p.id);
        const isNew = !existingProd;
        const isModified = existingProd && JSON.stringify(sanitizeProductForFirestore(existingProd)) !== JSON.stringify(sanitizeProductForFirestore(p));

        if (isNew || isModified) {
          console.log(`[updatePersistentProducts] Writing changed/new product to Firestore (${isNew ? 'NEW' : 'MODIFIED'}):`, p.id);
          batch.set(doc(db, 'products', p.id), sanitizeProductForFirestore(p));
          changesCount++;
        }
      }
      ...
      if (changesCount > 0) {
        await batch.commit();
        console.log('[updatePersistentProducts] Firestore batch commit success! Operations:', changesCount);
      } else {
        console.log('[updatePersistentProducts] No changes detected. Skipping Firestore commit.');
      }
    } catch (error: any) {
      // Firestore write failed — localStorage backup is already saved
      console.error('[updatePersistentProducts] Firestore product write failed:', error);
      alert(`⚠️ Firestore Database write failed: ${error.message || error}\n\nYour changes were saved locally (to this browser only) but NOT to the cloud. Please make sure you are logged in with a valid admin account.`);
    }
  };
  ...
  const updatePersistentWhatsApp = async (phoneNo: string) => {
    ...
    try {
      ...
      console.error('Firestore WhatsApp config write failed:', error);
      alert(`⚠️ Firestore Database write failed: ${error.message || error}\n\nCould not save WhatsApp configurations to the cloud. Please verify your admin session.`);
    }
  };
  ...
  const updatePersistentOrders = async (updatedOrd: OrderDetails[]) => {
    // Capture previous order IDs before state update (avoids stale closure)
    ...
    try {
      const activeIds = new Set(updatedOrd.map(o => o.orderId));
      const batch = writeBatch(db);

      for (const oldId of previousIds) {
        if (!activeIds.has(oldId)) {
          batch.delete(doc(db, 'orders', oldId));
        }
      }
      for (const o of updatedOrd) {
        batch.set(doc(db, 'orders', o.orderId), o);
      }

      await batch.commit();
    } catch (error: any) {
      console.error('Firestore orders write failed:', error);
      alert(`⚠️ Firestore Database write failed: ${error.message || error}\n\nCould not save order updates to the cloud. Please verify your admin session.`);
    } finally {
      isWritingOrders.current = false;
    }
  };
  ...
  return (
    <div className="min-h-screen max-w-full overflow-x-hidden bg-neutral-50/50 text-neutral-900 selection:bg-black selection:text-white flex flex-col font-sans">
      ...
      <section id="essence" className="py-6 sm:py-8 bg-white text-center border-b border-neutral-100">
        <div className="max-w-3xl mx-auto px-4">
          <span className="text-[9px] uppercase tracking-[0.25em] text-neutral-400 font-mono font-bold block mb-2">THE REƎD STANDARD</span>
          <p className="font-sans font-light text-base sm:text-lg text-neutral-500 leading-relaxed max-w-xl mx-auto">
      ...
      <main id="collection" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12 lg:pt-8 lg:pb-14 flex-grow font-sans">
        
        {/* Dynamic Page Header Block based on selectedGender */}
        <div className="border-b border-neutral-100 pb-6 mb-12 text-left space-y-4">
          <div className="flex items-center gap-2">
            <span className="p-1 px-2.5 bg-neutral-900 rounded-sm text-[8px] font-mono font-bold uppercase text-white tracking-widest transition-colors">
              {selectedGender === 'all' ? 'FULL CATALOG' : selectedGender === 'sale' ? 'CLEARANCE SALE' : `${selectedGender.toUpperCase()}'S WEAR`}
            </span>
          ...
          <h2 className="text-3xl sm:text-4xl font-display font-black tracking-tight text-neutral-900 uppercase">
            {selectedGender === 'men' 
          ...
          <p className="text-xs sm:text-sm font-light text-neutral-500 max-w-3xl leading-relaxed">
            {selectedGender === 'men' 
              ...
              : selectedGender === 'sale'
              ? "Premium engineered garments available at a limited-time reduction. Final sale on exclusive structural profiles."
      ...
      <footer className="bg-white border-t border-neutral-150 py-12 text-center text-xs text-neutral-400 font-medium">
        <div className="max-w-7xl mx-auto px-4 space-y-6">
          <div className="flex flex-col items-center">
            <span className="text-xl font-serif tracking-[0.25em] font-bold text-black select-none">
              REƎD
            </span>
          </div>

          <p className="max-w-md mx-auto text-[10px] text-neutral-500 leading-relaxed">
          ...
          <div className="flex justify-center space-x-8 text-[10px] uppercase font-semibold tracking-wider text-neutral-500">
            <a href="#collection" className="hover:text-black transition-colors">Catalog</a>
            <a href="#essence" className="hover:text-black transition-colors">The Standard</a>
            <a href="#whatsapp-info" className="hover:text-black transition-colors">How it works</a>
          </div>

          <p className="text-[9px] font-mono text-neutral-400 font-medium">
