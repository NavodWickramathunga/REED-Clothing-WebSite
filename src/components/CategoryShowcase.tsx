...
export default function CategoryShowcase({ onSelectCategory, selectedCategory }: CategoryShowcaseProps) {
  ...
  return (
    <section id="categories-showcase" className="py-14 sm:py-24 lg:py-32 bg-neutral-50/50 dark:bg-neutral-900/40 border-b border-neutral-100 dark:border-neutral-850 select-none transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        
        {/* SECTION TITLE */}
        <div className="max-w-xl mx-auto space-y-3 mb-10 sm:mb-16">
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 bg-neutral-200/50 dark:bg-neutral-800/55 text-neutral-600 dark:text-neutral-400 rounded-full text-[8.5px] font-mono tracking-widest uppercase font-extrabold">
            <Layers className="w-3.5 h-3.5 text-neutral-500 dark:text-neutral-400" />
            <span>CATEGORIES FILTERS</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-display font-black text-neutral-900 dark:text-white tracking-tight uppercase">
            SHOP BY CATEGORY
          </h2>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 font-light tracking-wide max-w-sm mx-auto">
            ...
            return (
              <div
                ...
                className={`group relative aspect-[4/5] sm:aspect-[3/4] md:aspect-[4/5] rounded-xl overflow-hidden cursor-pointer transition-all duration-350 ${
                  isCurrentlySelected 
                    ? 'ring-4 ring-black dark:ring-amber-400 shadow-lg scale-[1.01]' 
