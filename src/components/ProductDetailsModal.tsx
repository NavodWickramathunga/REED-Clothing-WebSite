...
export default function ProductDetailsModal({
  ...
  // Interactive product feedback states
  ...
  const handleSubmitReview = () => {
    if (!submitName.trim() || !submitComment.trim()) {
      alert('Please fill out your name and a comment review.');
      return;
    }
    const newReview: ProductReview = {
      id: 'rev-' + Math.floor(100000 + Math.random() * 900005),
      rating: submitRating,
      comment: submitComment.trim(),
      author: submitName.trim(),
    ...
    // Optimistic / local list update so it adds directly in modal window too!
    if (!product.reviews) {
      product.reviews = [];
    }
    product.reviews = [newReview, ...product.reviews];

    // Reset fields & set alerts
    setSubmitRating(5);
    setSubmitName('');
    setSubmitComment('');
    setReviewAlert('Review successfully submitted! Thank you for the verified feedback.');
    setTimeout(() => setReviewAlert(''), 4500);
  };
      ...
      >
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 md:overflow-y-auto flex flex-col md:max-h-[92vh] bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
          ...
          <div className="space-y-4 border-t border-neutral-100 dark:border-neutral-850 pt-4 flex-grow">
            ...
            <div className="space-y-2 pb-1" id="swatch-preview-section">
              <div className="flex items-center justify-between">
                <h4 className="text-[9px] uppercase tracking-widest text-neutral-400 dark:text-neutral-500 font-mono font-bold">
              ...
              <div className="flex flex-wrap gap-2.5">
                {allProducts && allProducts.filter(p => p.name.split(' - ')[0] === product.name.split(' - ')[0]).length > 1 ? (
                  // Multi-Product variants: Group by base shirt name and present as linked sibling swatches
                  allProducts.filter(p => p.name.split(' - ')[0] === product.name.split(' - ')[0]).map((variant) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative w-8 h-8 rounded-full border flex items-center justify-center transition-all cursor-pointer ${
                          isSelected
                            ? 'border-neutral-900 dark:border-amber-400 ring-2 ring-neutral-900/10 dark:ring-amber-400/30 scale-110 z-10'
                            : 'border-neutral-250 dark:border-neutral-800 hover:scale-105 hover:border-neutral-405 dark:hover:border-neutral-700'
                        }`}
                      ...
                      >
                        {isSelected && (
                          <span className="w-2.5 h-2.5 rounded-full bg-white drop-shadow mix-blend-difference" />
                        )}
                  ...
                  // Single Product internal colors fallback
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[9px] uppercase tracking-widest text-neutral-400 dark:text-neutral-500 font-mono font-bold">Select Size:</h4>
                <div className="flex items-center space-x-2">
                  <span className="text-[9px] text-neutral-450 dark:text-neutral-500 font-mono bg-neutral-100 dark:bg-neutral-900 px-1 py-0.5 rounded">Heavy Boxy Fit</span>
                  <button 
                    type="button"
            ...
            {!product.sizes.includes(selectedSize) ? (
              ...
              <div className="space-y-4 pt-4 border-t border-neutral-100 dark:border-neutral-850">
                ...
                <div className="flex flex-col gap-3 py-1">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-[9px] uppercase tracking-widest text-neutral-400 dark:text-neutral-550 font-mono font-bold mb-1">Quantity:</h4>
                      <div className="flex items-center border border-neutral-250 dark:border-neutral-800 rounded-none overflow-hidden h-9 bg-neutral-50 dark:bg-neutral-900">
                        ...
                        <span className="px-4 text-xs font-bold font-mono text-neutral-900 dark:text-neutral-154">{quantity}</span>
                        <button
                          type="button"
            ...
            {/* ⭐️ Interactive Reviews Section */}
            <div className="mt-8 pt-6 border-t border-neutral-100 dark:border-neutral-850 space-y-5 text-left font-sans text-xs">
              <div className="flex items-center justify-between">
                <h4 className="text-[10px] uppercase tracking-widest text-neutral-400 dark:text-neutral-500 font-mono font-bold">
                  Customer Reviews
                </h4>
                {product.reviews && product.reviews.length > 0 && (
                  <span className="text-[10px] font-mono font-bold text-amber-500 bg-neutral-100 dark:bg-neutral-900 px-2 py-0.5 rounded">
              ...
              {product.reviews && product.reviews.length > 0 ? (
                <div className="bg-neutral-50 dark:bg-neutral-900/40 p-4 border border-neutral-105 dark:border-neutral-800 rounded flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4">
                  <div className="text-center sm:text-left">
                    <div className="text-2xl font-black font-mono text-neutral-900 dark:text-white leading-tight">
                      {(product.reviews.reduce((acc, r) => acc + r.rating, 0) / product.reviews.length).toFixed(1)}
                    </div>
                    <div className="flex justify-center sm:justify-start gap-1 py-1 text-amber-500">
                      {[1, 2, 3, 4, 5].map((s) => {
                        const avg = product.reviews!.reduce((acc, r) => acc + r.rating, 0) / product?.reviews!.length;
                        return (
                          <Star 
                            key={s} 
                            className={`w-3.5 h-3.5 ${s <= Math.round(avg) ? 'text-amber-400 fill-amber-400' : 'text-neutral-200 dark:text-neutral-800'}`} 
                          />
                        );
                      })}
                    </div>
                    <p className="text-[10px] text-neutral-400 font-mono">
                      Based on {product.reviews.length} written verified feedbacks
                    </p>
                  </div>

                  <div className="w-full sm:max-w-xs space-y-1">
                    {[5, 4, 3, 2, 1].map((ratingVal) => {
                      const count = product.reviews!.filter(r => r.rating === ratingVal).length;
                      const pct = product.reviews!.length > 0 ? (count / product.reviews!.length) * 100 : 0;
                      return (
                        <div key={ratingVal} className="flex items-center gap-2 text-[10px] font-mono text-neutral-400">
                          <span className="w-10 text-left shrink-0">{ratingVal} Star</span>
                ...
                <div className="p-5 text-center border border-neutral-150 dark:border-neutral-850 bg-neutral-50/50 dark:bg-neutral-900/10 rounded">
                  <p className="text-neutral-500 font-serif italic text-[11px]">No feedback submitted for this item yet. Be the first to share your fit review!</p>
                </div>
              )}
              ...
              {product.reviews && product.reviews.length > 0 && (
                <div className="space-y-3.5 max-h-48 overflow-y-auto pr-1">
                  {product.reviews.map((rev) => (
                    <div key={rev.id} className="p-3 bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-850 rounded text-left">
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <span className="font-bold text-neutral-900 dark:text-neutral-105 block text-[11px] font-sans">
                            {rev.author || 'Anonymous'}
                          </span>
                          <span className="text-[8px] font-mono uppercase bg-emerald-500/10 text-emerald-600 dark:text-emerald-450 border border-emerald-500/15 px-1 py-0.5 rounded font-bold">
                            Verified Stitch Buyer
                          </span>
                        </div>
                        <span className="text-[8.5px] font-mono text-neutral-400 flex items-center gap-1">
                      ...
                      {/* stars rating row */}
                      <div className="flex gap-0.5 my-1.5 text-amber-500">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star 
                            key={s} 
                            className={`w-3 h-3 ${s <= rev.rating ? 'fill-amber-400 text-amber-400' : 'text-neutral-200 dark:text-neutral-800'}`} 
                          />
                        ))}
                      </div>

                      <p className="text-[11px] text-neutral-600 dark:text-neutral-350 leading-relaxed font-sans">
                        {rev.comment}
                      </p>
                    </div>
                  ))}
              ...
              {/* Review submit form */}
              <div className="p-4 border border-neutral-150 dark:border-neutral-850 rounded bg-neutral-50/60 dark:bg-neutral-900/30 space-y-3">
                <h5 className="font-bold font-mono tracking-tight text-[10px] uppercase text-neutral-700 dark:text-neutral-300">
                ...
                <div className="space-y-2.5">
                  {/* Star Rating Select */}
                  <div>
                    <label className="block text-[8px] font-mono uppercase text-neutral-400 font-bold mb-1">Select Rating:</label>
                    <div className="flex gap-1.5">
                      {[1, 2, 3, 4, 5].map((starVal) => (
                        <button
                          ...
                          className="hover:scale-115 transition-all text-amber-400 cursor-pointer"
                          title={`${starVal} Star`}
                        >
                          <Star 
                            className={`w-5 h-5 ${starVal <= submitRating ? 'fill-amber-400 text-amber-400' : 'text-neutral-200 dark:text-neutral-850 hover:text-amber-300'}`} 
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div>
                      <label className="block text-[8px] font-mono uppercase text-neutral-400 font-bold mb-1">Your Full Name:</label>
                      <input
                        type="text"
                        required
                        placeholder="E.g. Shamil de Silva"
                        value={submitName}
                        onChange={(e) => setSubmitName(e.target.value)}
                        className="w-full px-2.5 py-2 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded font-sans text-xs focus:ring-1 focus:ring-black outline-none block text-neutral-950 dark:text-neutral-50"
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-[8px] font-mono uppercase text-neutral-400 font-bold mb-1">Constructive Comment:</label>
                    <textarea
                      required
                      placeholder="Share details about the embroidery quality, fit specs, fabric texture..."
                      value={submitComment}
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 bg-black dark:bg-amber-400 text-white dark:text-black font-mono text-[9px] font-bold tracking-widest uppercase cursor-pointer hover:bg-neutral-850 dark:hover:bg-amber-500 transition-colors"
                  >
                    Post Review Feedback
                  </button>
                  {reviewAlert && (
                    <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 mt-1">{reviewAlert}</p>
                  )}
                </div>
              </div>
            </div>
